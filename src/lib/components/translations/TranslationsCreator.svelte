<script lang="ts">
	...
	async function handleRefreshTranslation(subtitle: SubtitleClip, translation: string) {
		...
		const textarea = document.getElementById('textaera-translation-' + subtitle.id);
			...
			<div class="p-2 border-b-2 px-10 border-[#413f3f]">
				...
				<div class="flex flex-col mt-1">
					{#each Object.keys(subtitle.translations) as translation}
						<p class="font-bold w-full">{getEditionFromName(translation)?.language}:</p>
						...
						<div class="flex relative">
							...
							<span
								...
								id={'textaera-translation-' + subtitle.id}
								role="textbox"
								contenteditable
								bind:textContent={subtitle.translations[translation]}
								on:keydown={(e) => {
									// Si `enter` est pressé, on va à la prochaine textarea
									if (e.key === 'Enter') {
										e.preventDefault();
										handleValidation(e, subtitle, translation);
									}
								}}
							>
							</span>
							...
							<button
								...
								on:click={() => handleRefreshTranslation(subtitle, translation)}
							>
								<svg
